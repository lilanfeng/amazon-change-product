 ...
 * @author lilanfeng
 */
public class ProductInventoryServiceImpl implements ProductChangeService {

    private static final Log logger = LogFactory.get();

    private ProductInventoryWork productInventoryWork;

    public ProductInventoryServiceImpl(ProductInventoryWork work){
        this.productInventoryWork = work;
    }
    ...
    public void doProductChange(List<String> selectFileNameList, String outDirName, String cookie) {
        this.productInventoryWork.publishEvent(ConsoleUtil.console("doProductChange.getInputFileList.start！"));
        List<File> inputFileList = getInputFileList(selectFileNameList);
        if(CollUtil.isEmpty(inputFileList)){
            this.productInventoryWork.publishEvent(ConsoleUtil.console("获取输入的产品数量为空！"));
        }
        /**
        this.productInventoryWork.publishEvent(ConsoleUtil.console("doProductChange.getInputData.start！");
        List<InputProductInfo> inputProductInfoList = getInputData(inputFileList);
        this.productInventoryWork.publishEvent(ConsoleUtil.console("本次获取输入的产品的数量为："+CollUtil.size(inputProductInfoList));
        List<InputProductInfo> outProductInfoList = changeProduct(textArea,inputProductInfoList,cookie);
        this.productInventoryWork.publishEvent(ConsoleUtil.console("本次转换有效的产品的数量为："+CollUtil.size(outProductInfoList));
    ...
    private void changeProductByFile(List<File> inputFileList,String outDirName,String cookie){
        ...
        inputFileList.forEach(file -> {
            ...
            if(isSecond.get() && App.config.getFileTimeout(App.config.getCountryWeb()) > 0){
                try {
                    this.productInventoryWork.publishEvent(ConsoleUtil.console( "本次文件间隔为：" + App.config.getFileTimeout(App.config.getCountryWeb()) +" ms"));
            ...
            if (productInventoryWork.isStop()) {
                // 完成一个文件后才取消任务
                productInventoryWork.publishEvent(ConsoleUtil.console("一个文件处理开始，开始取消处理！"));
                productInventoryWork.stopWork();
                return;
            }

            List<InputProductInfo> inputProductInfoList = getInputDataByFile(file, existAsinMap);
            this.productInventoryWork.publishEvent(ConsoleUtil.console(
    ...
    private List<InputProductInfo> changeProduct(List<InputProductInfo> list, String cookie,int totalCount,List<InputProductInfo> unSellProductInfoList) {
        ...
        list.stream().forEach(inputProductInfo -> {
            ...
            //boolean isSell = HttpHelpUtil.getChangeProduct(productInventoryWork,inputProductInfo, cookie,outInfo);
            boolean isSell  = false;
            if (productInventoryWork.isStop()) {
                productInventoryWork.publishEvent(ConsoleUtil.console("取消进行中，等本文件解析完成就停止！"));
            }
            if (isSell) {
                outCount.getAndIncrement();
                outInfo.setId(outCount.get());
                outProductInfoList.add(outInfo);
                this.productInventoryWork.publishEvent(ConsoleUtil.console("Suc:"+outInfo));
            }else {
